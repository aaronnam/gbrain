...
As of v0.22.4, local commits are expected to cover:
...
Interpretation:
- Clean `aaron/v0.22.4-local` ahead of `origin/master` by local commits = healthy.
- Dirty working tree on `aaron/v0.22.4-local` = inspect; it may be an in-progress local patch, not an upgrade failure.
...
```bash
git switch aaron/v0.22.4-local 2>/dev/null || git switch -c aaron/v0.22.4-local
...
```bash
git push -u aaron aaron/v0.22.4-local
```
...
- **Aaron-specific operational preference:** keep it on `aaron/v0.22.4-local`.
