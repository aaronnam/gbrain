...
ambiguous_with?}`. `gbrain check-resolvable` runs the structural layer by default. The
`gbrain routing-eval --llm` flag is reserved for a future LLM tie-break layer; today it
parses successfully but makes no model calls. False positives (wrong skill matched),
